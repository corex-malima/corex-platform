...
3. Crear credentials file con la cuenta de servicio del dominio (no la tuya):
   ...
   ```bash
   sudo install -m 600 -o root -g root /dev/null /etc/samba/corex-svc.cred
   sudo tee /etc/samba/corex-svc.cred > /dev/null <<EOF
   username=svc_corex
   password=...
   domain=GRUPO-MALIMA
   EOF
