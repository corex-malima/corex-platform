...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listCurrentLaboratoryProducts();
    ...
    return handleApiError(error, "No se pudo cargar el maestro de Laboratorio.");
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim()
    ...
    || "local";
  const rl = checkRateLimit(`laboratorio-recetas:${ip}`, 20, 60_000);
  if (!rl.allowed) {
    return jsonError(
  ...
  try {
    ...
    const data = await createLaboratoryProduct(payload, actorId);
    ...
    return handleApiError(error, "No se pudo crear el producto de Laboratorio.");
  }
}
