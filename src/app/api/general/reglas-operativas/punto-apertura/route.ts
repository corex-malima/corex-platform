...
import {
  createGeneralOpeningTargetRule,
  getGeneralOpeningTargetRuleModuleData,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getGeneralOpeningTargetRuleModuleData();
    ...
    return handleApiError(error, "No se pudo cargar el registro de punto de apertura.");
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rl = checkRateLimit(`general-opening-target-rules:${readRequesterIp(request)}`, 20, 60_000);
  if (!rl.allowed) {
    return jsonError(
  ...
  try {
    ...
    const data = await createGeneralOpeningTargetRule(parsed.data, actorId);
    ...
    return handleApiError(error, "No se pudo registrar la regla de punto de apertura.");
  }
}
