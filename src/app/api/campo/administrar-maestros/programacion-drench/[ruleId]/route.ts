...
import { deleteDrenchProgramRule, updateDrenchProgramRule } from "@/lib/campo-drench-program";
...
export async function DELETE(
  ...
  try {
    const { ruleId } = await context.params;
    const actorId = (await getSession()) ?? "corex_campo_drench_ui";
    const data = await deleteDrenchProgramRule(decodeURIComponent(ruleId), actorId);
    ...
    return handleApiError(error, "No se pudo eliminar la semana de drench.");
  }
}
