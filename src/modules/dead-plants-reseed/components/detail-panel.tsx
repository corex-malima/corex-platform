"use client";

import { LoaderCircle, Save, ShieldAlert } from "lucide-react";

import { EmptyState } from "@/shared/data-display/empty-state";
import { MetricTile } from "@/shared/data-display/metric-tile";
import { Button } from "@/shared/ui/button";
import { Input } from "@/shared/ui/input";
import { ScrollFadeTable } from "@/shared/tables/scroll-fade-table";
import { formatDate, formatDateTime, formatInteger } from "@/shared/lib/format";
import type { DeadPlantsReseedLoadDetail } from "@/lib/dead-plants-reseed";

function parseCountInput(value: string) {
  const numericValue = Number(value);
  return Number.isFinite(numericValue) ? Math.max(0, Math.floor(numericValue)) : 0;
}

export function DetailPanel({
  canWrite,
  countLabel,
  detail,
  values,
  setValues,
  changeReason,
  setChangeReason,
  isLoading,
  error,
  onSave,
  isSaving,
}: {
  canWrite: boolean;
  countLabel: string;
  detail?: DeadPlantsReseedLoadDetail;
  values: Record<string, number>;
  setValues: (values: Record<string, number>) => void;
  changeReason: string;
  setChangeReason: (value: string) => void;
  isLoading: boolean;
  error?: Error;
  onSave: () => void;
  isSaving: boolean;
}) {
  const changedCount = detail?.rows.filter((row) => values[row.eventId] !== undefined && values[row.eventId] !== row.value).length ?? 0;

  if (isLoading) {
    return <div className="h-72 animate-pulse rounded-[18px] bg-muted" />;
  }

  if (error) {
    return (
      <p className="rounded-[16px] border border-destructive/30 bg-destructive/8 px-4 py-3 text-sm text-destructive">
        {error.message}
      </p>
    );
  }

  if (!detail) {
    return <EmptyState label="Selecciona una carga del panel izquierdo para revisar o editar." />;
  }

  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3">
        <MetricTile label="Bloque" value={detail.summary.blockId} hint={formatDate(detail.summary.workDate)} />
        <MetricTile label="Camas" value={formatInteger(detail.summary.bedCount)} hint={detail.summary.actorId ?? "sin actor"} />
        <MetricTile label="Total" value={formatInteger(detail.summary.totalValue)} hint={detail.summary.loadedAt ? formatDateTime(detail.summary.loadedAt) : "sin fecha"} />
      </div>

      {!canWrite ? (
        <div className="flex items-center gap-2 rounded-[16px] border border-amber-300/40 bg-amber-100/40 px-4 py-3 text-sm text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
          <ShieldAlert className="size-4" aria-hidden="true" />
          Tu rol permite consulta, no edicion.
        </div>
      ) : null}

      {canWrite ? (
        <div className="space-y-2">
          <label htmlFor="dead-reseed-change-reason" className="text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
            Motivo de cambio
          </label>
          <Input
            id="dead-reseed-change-reason"
            value={changeReason}
            onChange={(event) => setChangeReason(event.target.value)}
            placeholder="Opcional. Si queda vacio se usara el motivo automatico de edicion."
          />
        </div>
      ) : null}

      <ScrollFadeTable className="border border-border/70">
        <table className="min-w-[700px] w-full border-separate border-spacing-0 text-sm">
          <thead className="sticky top-0 z-20 bg-card/95 backdrop-blur">
            <tr>
              <th className="border-b border-r border-border/70 px-3 py-3 text-left font-semibold">Cama</th>
              <th className="border-b border-r border-border/70 px-3 py-3 text-left font-semibold">Ciclo</th>
              <th className="border-b border-border/70 px-3 py-3 text-right font-semibold">{countLabel}</th>
            </tr>
          </thead>
          <tbody>
            {detail.rows.map((row, index) => (
              <tr key={row.eventId} className={index % 2 === 0 ? "bg-background/84" : "bg-background/70"}>
                <td className="border-b border-r border-border/50 px-3 py-2.5 font-medium">{row.bedPosition}</td>
                <td className="border-b border-r border-border/50 px-3 py-2.5 text-xs text-muted-foreground">{row.cycleKey}</td>
                <td className="border-b border-border/50 px-3 py-2.5 text-right">
                  <Input
                    type="number"
                    min={0}
                    step={1}
                    value={values[row.eventId] ?? row.value}
                    disabled={!canWrite}
                    onChange={(event) => setValues({ ...values, [row.eventId]: parseCountInput(event.target.value) })}
                    className="ml-auto w-28 text-right tabular-nums"
                  />
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </ScrollFadeTable>

      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          {formatInteger(changedCount)} camas con cambios. El historico se conserva invalidando el registro anterior.
        </p>
        <Button onClick={onSave} disabled={!canWrite || isSaving || changedCount === 0}>
          {isSaving ? <LoaderCircle className="size-4 animate-spin" aria-hidden="true" /> : <Save className="size-4" aria-hidden="true" />}
          Guardar cambios
        </Button>
      </div>
    </div>
  );
}
