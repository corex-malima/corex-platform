    ...
    <ChartSurface title={title} subtitle="Porcentaje y cantidad por respuesta. Click para ver personas.">
      {groups.length ? (
        <div className="grid gap-5 lg:grid-cols-[170px_1fr]">
          <div className="mx-auto grid size-40 place-items-center rounded-full border border-border/70 shadow-inner" style={{ background: `conic-gradient(${gradient})` }}>
            <div className="grid size-24 place-items-center rounded-full bg-card text-center text-xs font-semibold shadow-sm">
              {formatInteger(total)}
          ...
          <div className="grid gap-2">
            {topGroups.map((group, index) => {
              const ratio = total ? group.count / total : 0;
              return (
                <button key={group.label} type="button" onClick={() => onSelect(group)} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[14px] border border-border/60 bg-background/70 px-3 py-2 text-left transition hover:border-primary/35 hover:bg-muted/45">
                  ...
                  <span className="text-xs font-semibold tabular-nums">{formatPercent(ratio, { input: "ratio", minimumFractionDigits: 0, maximumFractionDigits: 0 })} · {formatInteger(group.count)}</span>
                </button>
              );
            })}
          </div>
  ...
  return (
    <ChartSurface title={title} subtitle={subtitle}>
      {groups.length && columns.length ? (
        <div className="overflow-x-auto rounded-[18px] border border-border/60 bg-background/55 p-2">
...
export function SocialWorkerContingencyCard({
  ...
  return (
    <ChartSurface title="Trabajadora social vs motivo, cumplimiento y antigüedad" subtitle="Matriz resumen por TS: dominante, volumen y cumplimiento promedio.">
      {groups.length ? (
        <div className="overflow-x-auto rounded-[18px] border border-border/60 bg-background/55 p-2">
          <table className="w-full min-w-[900px] border-separate border-spacing-0 text-xs">
            <thead>
              <tr>
                ...
                <Th>Cumplimiento dominante</Th>
                <Th>Antigüedad dominante</Th>
                <Th align="right">Cumpl. prom.</Th>
              </tr>
            </thead>
            <tbody>
              {groups.map((group) => {
                const topReason = group.buckets[0];
                const topCompliance = group.complianceBuckets[0];
                const topTenure = group.tenureBuckets[0];
                return (
                  <tr key={group.label} className="cursor-pointer hover:bg-muted/35" onClick={() => onSelect(group)}>
                    <Td strong>{group.label}</Td>
                    <Td align="right" strong>{formatInteger(group.count)}</Td>
                    <SummaryTd bucket={topReason} />
                    <SummaryTd bucket={topCompliance} />
                    <SummaryTd bucket={topTenure} />
                    <Td align="right" strong>
                      <span className={toneClass(getComplianceTone(group.avgCompliance))}>
...
function SummaryTd({ bucket }: { bucket?: CrossGroup["buckets"][number] }) {
  return (
      ...
      {bucket ? (
        <div className="flex min-w-0 items-center justify-between gap-3">
          ...
          <span className="shrink-0 rounded-full bg-muted px-2 py-0.5 text-[11px] font-semibold tabular-nums">
            {formatInteger(bucket.count)} · {formatPercent(bucket.ratio, { input: "ratio", minimumFractionDigits: 0, maximumFractionDigits: 0 })}
          </span>
        </div>
      ) : "-"}
    </td>
